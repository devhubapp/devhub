  ...
  async componentDidMount() {
    try {
      const response = await fetch(
        'https://api.github.com/notifications?all=1&access_token=fae0e8d5d55b71afb4c59d6abb89fce457c48160',
      )
      const notifications = await response.json()
      if (Array.isArray(notifications)) this.setState({ notifications })
    } catch (error) {
      console.error(error)
